"""NPZ -> FBX retargeter. Kimodo motion -> animated Mixamo-rigged FBX.

No Blender. Uses the Autodesk FBX SDK via ``fbxsdkpy``.

This is a port of jtydhr88/ComfyUI-Kimodo kimodo_retarget_fbx.py, adapted to
take raw numpy arrays directly (our in-process path keeps NPZ in memory) and
to be reusable inside our FastAPI pipeline. The retarget math is the same:
  target_world_rot[f, j] = source_world_rot[f, j] * (source_rest_rot[j]^-1 * target_rest_rot[j])
  target_local_rot[f, j] = parent_target_world_rot[f, parent(j)]^-1 * target_world_rot[f, j]
Root translation for the hips bone is re-expressed in the target rig's rest
frame and scaled by the skeleton-height ratio.

Credit: logic mirrored from ComfyUI-Kimodo (Apache-2.0), which itself follows
a standard retarget formulation for T-pose-to-T-pose bone correspondence.
"""
...
# Mixamo bones are prefixed "mixamorig:" on import from Adobe; the matcher
# strips the prefix case-insensitively when looking up target bones, so
# differently-prefixed rigs (e.g. "mixamorig1:") still match.
SOMA_TO_MIXAMO: dict[str, str] = {
    ...
    # Left hand fingers (SOMA77 expansion)
...
def _require_fbx_sdk():
    try:
        import fbx  # noqa: F401
    except ImportError as e:
        raise FbxSdkMissing(
            "FBX SDK (fbxsdkpy) is required for FBX export. "
            "Install from the Inria index, e.g.:\n"
...
# ---------------------------------------------------------------------------
# Quaternion helpers. Quaternions are stored as (w, x, y, z); scipy uses
# (x, y, z, w), so conversions happen at the boundary.
# ---------------------------------------------------------------------------
...
def _mat3_to_quat_wxyz(m33: np.ndarray) -> np.ndarray:
    # scipy expects M @ v convention; kimodo's global_rot_mats are standard so
    # we pass m33 directly (no transpose) — the upstream NPZ docs document the
    # rotation matrices as rotating column vectors.
...
def _fbx_mat_rot_to_quat_wxyz(fbx_mat) -> np.ndarray:
    """Extract rotation quaternion from a 4x4 FBX matrix.

    FBX uses row-major (v*M) convention so we transpose the 3x3 before
    handing to scipy.
    """
...
# ---------------------------------------------------------------------------
# Source skeleton (Kimodo NPZ)
# ---------------------------------------------------------------------------

class _Bone:
    """Source (Kimodo) or target (FBX) bone metadata + per-frame animation."""
    ...
    def __init__(self, name: str) -> None:
        ...
        self.rest_rotation = np.array([1.0, 0.0, 0.0, 0.0])  # identity wxyz
...
class _Skeleton:
    def __init__(self, name: str = "skeleton") -> None:
        self.name = name
        self.bones: dict[str, _Bone] = {}  # keyed lowercase
        self.node_rest_rotations: dict[str, np.ndarray] = {}  # by ORIGINAL case
        self.fps = 30.0
        self.frame_start = 0
        self.frame_end = 0
    ...
    def find(self, name: str) -> _Bone | None:
        ...
        # Try prefix-stripped lookup (e.g. "mixamorig:hips" <-> "hips",
        # "mixamorig1:hips" also OK).
        if ":" in lo:
            stripped = lo.split(":")[-1]
...
def _build_source_skeleton(
    ...
    """Build a _Skeleton from Kimodo's NPZ outputs.

    SOMA's standard T-pose is defined such that all global rotations are
    identity. That is what the model emits on training data in "standard
    T-pose frame"; our caller's responsibility is to ensure the data coming
    in matches this convention (the model does by default). We therefore
    set every bone's ``rest_rotation`` to identity.
    """
...
# ---------------------------------------------------------------------------
# Target skeleton (FBX). Walks the scene graph starting at the root node.
# ---------------------------------------------------------------------------
...
def _collect_target_skeleton(node, scene, skel: _Skeleton, parent_name: str | None = None) -> None:
    ...
    if attr:
        ...
        # 3 = Skeleton, 4 = LimbNode, 2 = Null (treat as bone if anchored to hierarchy)
        if atype in (3, 4):
            is_bone = True
        elif atype == 2 and (node.GetChildCount() > 0 or parent_name):
            is_bone = True

    # Broaden via name (catches engine-specific rigs that lack an FBX attribute).
    ...
    # Prefer BindPose if available.
    for i in range(scene.GetPoseCount()):
        pose = scene.GetPose(i)
        if pose and pose.IsBindPose():
            idx = pose.Find(node)
            if idx != -1:
                skel.node_rest_rotations[node_name] = _fbx_mat_rot_to_quat_wxyz(pose.GetMatrix(idx))
                break
    ...
    if is_bone:
        ...
        if existing:
            # Keep the more authoritative (FBX skeleton attribute) record.
    ...
    if is_bone:
        ...
        # BindPose head override when present and non-zero.
        for i in range(scene.GetPoseCount()):
            pose = scene.GetPose(i)
            if pose and pose.IsBindPose():
                idx = pose.Find(node)
...
def _load_target_fbx(path: str):
    """Return (manager, scene, target _Skeleton)."""
    _require_fbx_sdk()
    ...
    # Sample rest pose with no active anim stack.
...
def _retarget(
    ...
    """Return (rotations, translations) keyed by target bone name, per frame.

    rotations[bone_name][frame] = local quaternion wxyz
    translations[bone_name][frame] = 3-vec in target bone's parent frame
    """
    ...
    # 1. build active bone-pair table and per-pair rest offset.
    ...
    for s, t, off in active:
        ...
        # Root translation: only on hips-equivalent.
        #
        # The ComfyUI-Kimodo formula computed (target bind pos in source units)
        # + (source hips world pos at frame f), treating those as linearly
        # additive. That assumes source rest pose = source posed frame 0 —
        # true for Mixamo, but NOT for Kimodo. Kimodo's `neutral_joints[root]`
        # is (0, 0, 0) by convention (root-at-origin) while `posed_joints[f,
        # root]` puts the hips at standing height (~0.9 m world y). The old
        # formula stacked that 0.9 m on top of Mixamo's bind height (~100 cm),
        # producing a character floating hundreds of cm above the floor.
        #
        # Correct formulation: target hips at frame f = target bind + scaled
        # source-frame-f displacement FROM source frame 0. At frame 0 the
        # delta is zero, so the mesh sits exactly at its bind position (which
        # is where the skin was weighted) — no double-counting.
        if "hips" in s.name.lower() or "hips" in t.name.lower():
            ...
            for f in frames:
                ...
                delta_source = s_p - s_p_0            # source units (meters)
                delta_target = delta_source * scale   # target units (cm, typically)
                if not preserve_root_xz:
                    # In-place animation: zero horizontal drift so the character
                    # stays pinned at its bind-pose hips position. Vertical (Y)
                    # motion is preserved so jump/crouch/fall verticality and
                    # walking bounce still animate. This is the Mixamo / Unity
                    # "humanoid root-motion OFF" convention that drives the
                    # character world position from game code, not animation.
                    delta_target = np.array([0.0, delta_target[1], 0.0])
                if yaw_offset_deg != 0.0:
                    delta_target = yaw_rot.apply(delta_target)
                # Transform the world-space delta into the parent's local
                # frame so the FBX translation curve is correct even when
                # the hips bone has a non-identity parent (rare for Mixamo
                # — hips is usually directly under scene root).
...
# ---------------------------------------------------------------------------
# Write animation onto the FBX scene.
# ---------------------------------------------------------------------------
...
def _apply_animation(
    ...
    # range. Without this, the stack inherits whatever span the input FBX's
    # original animation had (usually a mismatch) — three.js FBXLoader reads
    # that stale span, reports the clip as longer than it really is, and
    # plays past our last keyframe leaving the character frozen or jumping
    # to origin for the remainder of the loop. See user report 2026-04-23.
    frame_indices = set()
    ...
    if frame_indices:
        ...
        # Also update the scene's global timeline span so the exported FBX
        # carries a consistent duration at every level (stack, layer, scene).
        try:
...
def _save_fbx(manager, scene, output_path: str) -> None:
    """Save the scene as an FBX. Enables texture+material embedding so the
    output FBX plays with the correct skin/materials in Unity without needing
    any sidecar texture files. Fixes the "character imports as pure white"
    symptom seen with the old Blender-based pipeline.
    """
    import fbx
    ...
    # Texture/material embedding — the important bit. EXP_FBX_EMBEDDED is the
    # master switch; the string props are a belt-and-suspenders fallback for
    # fbxsdkpy builds where the module constant isn't exported.
...
def retarget_npz_to_fbx(
    *,
    ...
    Inputs are the raw numpy arrays as emitted by Kimodo (after stripping
    the leading num_samples dim). See ``app.kimodo_runtime.KimodoRuntime``.

    The target FBX is loaded, its animation stack replaced, and written to
    ``output_fbx_path`` preserving the mesh/skin/materials from the input.

    ``preserve_root_xz`` controls whether the horizontal (XZ) component of
    the hips translation is written into the output FBX. Default is False —
    animations are "in-place," the character stays pinned at the bind-pose
    hips position and game code drives world translation. Set to True to
    get a root-motion clip (character actually translates across the scene
    as in Kimodo's generated motion). Vertical (Y) translation is always
    preserved so jumps/crouches/walk-bounce still animate.
