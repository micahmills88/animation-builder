 ...
 *
 * Patched in animation-builder/docker/patches/ for aarch64 builds.
 * The only change vs upstream: the immintrin.h include now branches on host
 * architecture. On aarch64 we pull in simde (SIMD Everywhere) headers with
 * SIMDE_ENABLE_NATIVE_ALIASES so the rest of the file (and Matrix.inl,
 * Quaternion.inl, etc.) keep using __m128 / _mm_* exactly like x86 — simde
 * maps each call to ARM NEON. Zero changes needed elsewhere.
 *
 * Apt: libsimde-dev (Ubuntu 24.04 main).
 */
